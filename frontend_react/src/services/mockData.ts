...
function img(seed: number): string {
  // Placeholder images; in real app use CDN or backend URLs
  const pics = [
...
const MOCK: Recipe[] = Array.from({ length: 24 }).map((_, i) => {
...
export const mockApi = {
  ...
  async request<T>(path: string): Promise<T> {
    await new Promise((r) => setTimeout(r, 250)); // mimic latency
    const url = new URL(path, 'http://example.local'); // base ignored
    ...
    const match = url.pathname.match(/^\\/recipes\\/(\\w+)/);
    if (match) {
      const id = match[1];
